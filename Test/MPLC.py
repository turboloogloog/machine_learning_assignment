...
mlp = MLPClassifier(activation='identity', alpha=0.0001,hidden_layer_sizes=(20, 10),random_state=1,solver='adam') #best precision
					#(activation = 'tanh', solver='lbfgs', alpha=0.00001, hidden_layer_sizes=(30,30), random_state=1) #default
					#(activation='tanh', alpha=0.01,hidden_layer_sizes=(20, 10),random_state=2,solver='lbfgs') #best recall
mlp.fit(train_x, train_y)
predicted_y = mlp.predict(test_x)
#scores = cross_val_score(mlp, train_x, train_y, cv=10) #10-fold cross validation 
#print("MLPC 10-fold cross validation Accuracy: %0.3f (+/- %0.3f)" % (scores.mean(), scores.std() * 2))
...
plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True, title='Normalized Confusion Matrix for Neural network')
...
print('MLPC accuracy: {0}, MLPC precision: {1}, MLPC recall: {2}, MLPC Fscore: {3}'.format(accuracy, metric[0], metric[1], metric[2]))
