...
Bg_dcTree = BaggingClassifier(tree.DecisionTreeClassifier(), n_estimators = 200)
Bg_dcTree.fit(train_x, train_y)
predicted_y = Bg_dcTree.predict(test_x)

#scores = cross_val_score(Bg_dcTree, train_x, train_y, cv=10) #10-fold cross validation 
#print("Bg_dcTree 10-fold cross validation Accuracy: %0.3f (+/- %0.3f)" % (scores.mean(), scores.std() * 2))
...
plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True, title='Normalized Confusion Matrix for Bagging Decision Tree')
...
print('Bagging Decision Tree accuracy: {0}, Bagging Decision Tree precision: {1}, Bagging Decision Tree recall: {2}, Bagging Decision Tree Fscore: {3}'.format(accuracy, metric[0], metric[1], metric[2]))
