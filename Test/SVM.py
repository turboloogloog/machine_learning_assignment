...
#scores = cross_val_score(svm, train_x, train_y, cv=10) #10-fold cross validation 
#print("SVM Accuracy: %0.3f (+/- %0.3f)" % (scores.mean(), scores.std() * 2))
...
plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True, title='Normalized Confusion Matrix for SVM')
...
print('SVM accuracy: {0}, SVM precision: {1}, SVM recall: {2}, SVM Fscore: {3}'.format(accuracy, metric[0], metric[1], metric[2]))
